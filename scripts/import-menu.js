...
function cleanMenuItem(text) {
    ...
    // =====================================
    // ODSTRÁNENIE CENY A VŠETKÉHO ZA ŇOU
    // =====================================
    ...
    result = result.replace(
        /[.,]?\d{1,2}(?:[.,]\d{1,2})?[.,]?(?:6[,.]90|9[,.]20)[0-9€%]*$/i,
        ""
    );

    // =====================================
    // ALERGÉNY ZLEPENÉ NA KONCI TEXTU
    // napr. "vňaťkou 137"
    // napr. "granko)1"
    // =====================================
    ...
    result = result
        ...
        .trim();
        result = result.replace(
        /([a-zá-ž)])(?:1,3|1,7|3,7|1,3,7|13,7|137)\s*$/i,
        "$1"
    );
